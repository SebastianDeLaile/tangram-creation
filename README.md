...
- **Collapsible left sidebar** — shapes grouped by category (reads `examples/index.json` directly, so any figure added there shows up automatically), each row showing its auto-computed difficulty as a star rating, an **A–Z / Difficulty sort toggle** (difficulty sorts easiest-first as a flat list), a theme picker, a color swatch per piece type, fill/outline toggle, corner rounding slider, and "Download JSON" / "Download card PDF" / "Download all shown (PDF)" buttons (plus a binding-gutter checkbox). Collapse it with the `«`/`»` button at the top to give the canvases more room.
...
- **PDF puzzle cards** (`web/src/pdf.ts`, jsPDF + svg2pdf.js, lazy-loaded on click) — "Download card PDF" exports the current figure as a **double-sided A5 card**: page 1 is the solid-black silhouette + title, category, and difficulty dots; page 2 is the colored solution **mirrored horizontally** so a duplex print (flip on the long edge) lands the answer behind its own silhouette. Each card auto-orients **portrait or landscape** to fit the tangram, sized for lamination and ring-binder storage. An optional **binding gutter** adds a blank hole-punch strip down one side (on the left of the front and, mirrored, the right of the back, so punched holes align). The tangram art is embedded as true vector; frame, crop marks, and difficulty dots are drawn with jsPDF. "Download all shown (PDF)" batch-exports every figure currently listed in the sidebar (respecting the category/search filter) as duplex cards in one file. See [`docs/PRINT_PLAN.md`](docs/PRINT_PLAN.md).

Deployed live at [tangram.delaile.com](https://tangram.delaile.com) via GitHub Pages — `.github/workflows/deploy.yml` builds and publishes on every push to `main`.
